...
export default class extends Vue {
    ...
    private async getList(){
        try {      
            ...
            const res = await getIconTableList(params);
            console.log(res,'6565656')
    ...
    private async handleSubmit() {
        await delIconTableCode(this.ids)
    ...
    private async handleRowUpdated(row: any, index: Number, done: any) {
        await updateIconTableCode(row)
        done();
        this.$message.success("修改成功");
        this.getList();
    }
    private async handleRowSave(row: any, done: any) {
        await addIconTableCode(row)
        done();
