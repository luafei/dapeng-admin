...
export default class extends Vue {
    ...
    private option = {
        ...
        column: [
            ...
            },
            {
                ...
                prop: 'busStationName',
                rules: [
                    {required: true, message:'请输入公交站点名称', trigger:'blur'}
                ]
            },
            {
                label: '视频编码',
                ...
                minWidth: 230,
                rules: [
                    {required: true, message:'请输入视频编码', trigger:'blur'}
                ]
            },
            {
                label: "客流预警阀值",
                prop: "peopleAlarmThreshold",
                rules: [
                    {required: true, message:'请输入客流预警阀值', trigger:'blur'}
                ]
            },
            {
                label: "经度",
    ...
    private async getList(){
        try {      
            ...
            const res = await getBusCodeList(params);
    ...
    private handleShowDelDialog(row: any): void {
        // console.log(row)
        this.delName = row.busStationName;
        this.ids = row.id+''
        this.showTipsDialog = true;
    }
    private async handleSubmit() {
        await delBusCode(this.ids)
    ...
    private submitUpload(file){
        this.$confirm('此操作将覆盖之前数据, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            const res = await importBusCode(file.raw);
            this.getList()
