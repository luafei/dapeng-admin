...
<el-card class="box-card">
    <div slot="header" class="clearfix">
        <span>{{$t('route.coordinateMapCode')}}</span>
    </div>
    <avue-crud 
    ...
    <tipsDialog v-if="showTipsDialog" :show.sync="showTipsDialog" title="删除码表" @closed="closed" @handle-submit="handleSubmit">
        <div class="dle-tip">
            ...
            是否删除{{$t('route.coordinateMapCode')}}记录：{{delName}} 的记录
        </div>
    </tipsDialog>
...
import {
    getCoordinateMapList,
    addCoordinateMapCode,
    updateCoordinateMapCode,
    delCoordinateMapCode,
    importIconTableCode,
    exportIconTableCode
...
export default class extends Vue {
    ...
    private option = {
        ...
        column: [
            ...
            },
            {
                ...
                rules: [
                    {required: true, message:'请选择是否转换', trigger:'change'}
                ]
            }
        ],
    };
    ...
    private async getList(){
        try {      
            ...
            const res = await getCoordinateMapList(params);
    ...
    private async handleSubmit() {
        await delCoordinateMapCode(this.ids)
    ...
    private async handleRowUpdated(row: any, index: Number, done: any) {
        await updateCoordinateMapCode(row)
        done();
        this.$message.success("修改成功");
        this.getList();
    }
    private async handleRowSave(row: any, done: any) {
        await addCoordinateMapCode(row)
        done();
