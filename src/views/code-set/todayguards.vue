...
export default class extends Vue {
    ...
    private option = {
        ...
        column: [
            ...
            },
            {
                ...
                minWidth: 300,
                rules: [
                    {required: true, message:'请输入办事处', trigger:'blur'}
                ]
            },
            {
                label: "姓名",
    ...
    private async getList(){
        try {      
            ...
            const res = await getTodayguards(params);
    ...
    private async handleRowUpdated(row: any, index: Number, done: any) {
        await updateTodayguards(row)
        done();
        this.$message.success("修改成功");
        this.getList();
    }
    private async handleRowSave(row: any, done: any) {
        await addTodayguards(row)
        done();
