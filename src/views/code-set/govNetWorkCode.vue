...
<template>
    <el-card class="box-card">
        <div slot="header" class="clearfix">
            <span>{{$t('route.govNetWorkCode')}}</span>
        </div>
         <avue-crud 
...
export default class extends Vue {
    ...
    private async getList(){
        try {      
            ...
            const res = await getGvoNetworkDicPageList(params);
    ...
    private handleShowDelDialog(row: any): void {
        // console.log(row)
        this.delName = row.networkName;
        this.ids = row.id+''
        this.showTipsDialog = true;
    }
    private async handleSubmit() {
        await delGvoNetworkDic(this.ids)
