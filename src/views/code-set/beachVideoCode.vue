...
<el-card class="box-card">
    <div slot="header" class="clearfix">
        <span>{{$t('route.beachVideoCode')}}</span>
    </div>
    <avue-crud 
...
import {
    getbeachVideoTableList,
    addbeachVideoTableCode,
    updatabeachVideoTableCode,
    delbeachVideoTableCode,
    importIconTableCode,
    exportIconTableCode
...
export default class extends Vue {
    ...
    private async getList(){
        try {      
            ...
            const res = await getbeachVideoTableList(params);
    ...
    private async handleSubmit() {
        await delbeachVideoTableCode(this.ids)
    ...
    private async handleRowUpdated(row: any, index: Number, done: any) {
        await updatabeachVideoTableCode(row)
        done();
        this.$message.success("修改成功");
        this.getList();
    }
    private async handleRowSave(row: any, done: any) {
        await addbeachVideoTableCode(row)
        done();
